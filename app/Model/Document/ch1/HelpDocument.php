...
// Home Start
$Stanzas_Index_Right = 'GlyTouCan 是国际的糖类结构数据库的网页界面。这个数据库免费开放，不经审核即可为通过独立信息水平实验验证的聚糖结构分配全球独一无二的编码。对于任意结构已经明晰的糖类，从单糖到完整的复合糖类，只要与原先存在的结构没有矛盾就可以被登记。

用户可以在库中搜索已登记糖类的结构和图形。此外， 登记了的用户可以对新发现的糖类结构进行登记并且获取其独一无二的ID，这个ID获准后可用于发表或其他数据库。

这个数据库由MEXT(日本文部科学部) 的集成数据库项目和JST(日本科学技术振兴机构)的相关数据库协调后进行一体化开发。';
$Stanzas_Index_FootBottom = 'Copyright © 2014 GlyTouCan';
// Home End

// Glycan Registration Start
$Registries_Index_Title ='注册多糖';
$Registries_Index_Left = '用GlycoCT condensed 格式输入多糖结构';
$Registries_Index_RightTitle = 'GlycoCT condensed 格式举例: 包含了重复单元的多糖的GlycoCT 格式.';
...
$Structures_StructureSearch_Title = '搜索结构';
//Left
$Structures_StructureSearch_Left [0] = '';
$Structures_StructureSearch_Left [1] = '搜索类型';
$Structures_StructureSearch_Left [2] = '通过输入多糖结构来搜索完全相同的结构.';
$Structures_StructureSearch_Left [3] = '搜索子结构';
$Structures_StructureSearch_Left [4] = '选择序列格式 :';
...
//Right
$Structures_StructureSearch_RightTitle [0] = '搜索类型';
$Structures_StructureSearch_RightTitle [1] = '通过输入多糖结构来搜索完全相同的结构.';
$Structures_StructureSearch_Right [1] = '通过输入多糖结构来搜索完全相同的结构.';
$Structures_StructureSearch_RightTitle [2] = '搜索子结构';
$Structures_StructureSearch_Right [2] = '搜索包含了输入的多糖结构的所有子结构.';
//Bottom
