...
// Home Start
$Stanzas_Index_Right = 'GlyTouCan 是國際的醣類結構數據庫的網頁界面。這個數據庫免費開放，不經審核即可為通過獨立信息水平實驗驗證的聚糖結構分配全球獨一無二的編碼。對於任意結構已經明晰的糖類，從單醣到完整的複合糖類， 只要與原先存在的結構沒有矛盾就可以被登記。

用戶可以在庫中搜索已登記糖類的結構和圖形。此外， 登記了的用戶可以對新發現的糖類結構進行登記並且獲取其獨一無二的ID， 這個ID獲准後可用於發表或其他數據庫。

這個數據庫由MEXT(日本文部科學部) 的集成數據庫項目和JST(日本科學技術振興結構)的相關數據庫協調後進行一體化開發。';
